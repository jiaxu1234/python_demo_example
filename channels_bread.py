...
# 寻找长度过长的标签
def toolong(tag):
    # 当前设置过长文本的阈值为4000，请根据需要调整
    too_long_lenth = 4000
    if tag.name in ['br', 'p', 'tr'] and len(list(tag.stripped_strings)) in [0, 1] and len(list(''.join(i for i in tag.parent.stripped_strings))) > too_long_lenth:
        return tag
...
def channel_soup_get(response_get, encoding):
    ...
    toolong_tag = channel_bsoup.find(toolong)
    if toolong_tag is not None:
        toolong_tag.parent.extract()
...
if __name__ == '__main__':
    ...
    # url = 'http://huzhou.58.com/zufang/30402045970372x.shtml'
